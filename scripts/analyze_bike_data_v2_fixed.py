...
OUTPUT_FILES = {
    ...
    'popular_stations_and_routes': 'popular_stations_and_routes_v2.png'
}
...
def analyze_demand_features(df):
    ...
    if len(key_features) > 1:
        print(\"正在生成散点图矩阵...\")
        pair_plot_df = df[key_features].sample(n=min(1000, len(df)), random_state=42) # 采样以提高性能
...
def visualize_popular_items(top_start_stations, top_end_stations, top_routes, top_n=TOP_N):
    ...
    fig.suptitle('共享单车热门站点与路线分析 (Version 2)', fontsize=26, y=0.96, weight='bold', color='#2C3E50')
    ...
    # 保存图表
    plt.savefig(OUTPUT_FILES['popular_stations_and_routes'], dpi=300, bbox_inches='tight', 
                facecolor='white', edgecolor='none', format='png', transparent=False)
    plt.close()
    print(f\"图表已保存至: {OUTPUT_FILES['popular_stations_and_routes']}\")
    print(\"--- 可视化图表生成完成 ---\\n\")


# --- 主执行逻辑 ---
if __name__ == \"__main__\":
    print(\"=== 共享单车数据分析脚本 (Version 2) ===\\n\")
