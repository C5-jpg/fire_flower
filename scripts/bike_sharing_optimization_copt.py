...
OUTPUT_DIR = r'C:\Users\hk\fire_flower'
...
def solve_problem_1(master_df, candidate_stations_to_close, C_total_initial, weight_cost, output_filename):
    ...
    try:
        ...
        model.setParam(GRB.Param.OutputFlag, 0) # 关闭求解日志
        model.solve()
        ...
        # 解F2最小
        model.setObjective(service_loss_expr, GRB.MINIMIZE)
        model.solve()
        if model.status == GRB.Status.OPTIMAL:
            Z2_min = model.objval
            # 固定当前解，求另一个目标的值
...
def solve_problem_2_robust(master_df, candidate_stations_to_close, C_total_initial, penalty_coefficient_P, output_filename):
    ...
    try:
        ...
        u = model.addVars(stations, vtype=GRB.CONTINUOUS, nameprefix="u")
        for i in stations:
